      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className={`relative w-full ${maxWidthClasses[maxWidth]} bg-white rounded-ag-lg border border-slate-200 shadow-modal z-10 overflow-hidden`}
          >
            {title && (
              <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                <div>
                  <h3 className="text-base font-semibold text-agText-primary">{title}</h3>
                  {description && <p className="text-xs text-agText-muted mt-0.5">{description}</p>}
                </div>
                <button
                  onClick={onClose}
